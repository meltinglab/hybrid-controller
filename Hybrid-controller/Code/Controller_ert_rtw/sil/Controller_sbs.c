...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("ControllerModel");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
static void mdlSetupRuntimeResources(SimStruct *S)
{
...

  {
    ...
    if (error != NULL) {
      ...
      ssSetErrorStatus( S,
                       "Failed to determine the installed In-the-Loop version for comparison against the In-the-Loop s-function version (release 9.9 (R2020b)_14). To avoid this error, remove the In-the-Loop s-function from your MATLAB path (e.g. delete it or move to a clean working directory).");
      return;
    }                                  /* if */
    ...
    if (strcmp(installVersion, "9.9 (R2020b)_14") != 0) {
      ssSetErrorStatus( S,
                       "The In-the-Loop s-function is incompatible with the installed In-the-Loop version (see ver('matlab')); it was generated for release 9.9 (R2020b)_14. To avoid this error, remove the In-the-Loop s-function from your MATLAB path (e.g. delete it or move to a clean working directory)");
      return;
    }                                  /* if */
  ...
  {
    {
      ...
      /* dispatch command to the target */
      if (commandDispatch(S, IOBufferPtr, 49)!=XILHOSTAPPSVC_SUCCESS) {
        return;
      }                                /* if */
